    ...
    public static function execute()
    {
        try {
            ...
            if (request()->has('payment_status') && request()->input('payment_status')) {
                $data = $data->where('payment_status', request()->input('payment_status'));
            }

            if (request()->has('search') && request()->input('search')) {
                $searchKey = request()->input('search');
                $data = $data->where(function ($q) use ($searchKey) {
                    $q->where('due_amount', 'like', '%' . $searchKey . '%')
                      ->orWhere('paid_amount', 'like', '%' . $searchKey . '%')
                      ->orWhere('remaining_amount', 'like', '%' . $searchKey . '%')
                      ->orWhere('for_month', 'like', '%' . $searchKey . '%')
