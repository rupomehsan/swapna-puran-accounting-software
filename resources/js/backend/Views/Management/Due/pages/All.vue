        ...
        <div class="card">
          <div class="card-header">
            <div class="row align-items-center">
              ...
              <!-- Reload, Generate Due and Filter Buttons -->
              <div class="col-12 col-md-3 text-md-right text-sm-left">
                ...
                <button
                  ...
                  title="Generate monthly dues for all active members"
                >
                  <i class="fa fa-bolt mr-1"></i>
                  {{ is_generating ? "Generating..." : "Generate Due" }}
                </button>
                <button
...
export default {
  ...
  methods: {
    ...
    generate_due: async function () {
      let con = await window.s_confirm(
        "এই মাসের সকল সক্রিয় সদস্যের জন্য বকেয়া তৈরি করবেন?"
      );
      if (!con) return;

      this.is_generating = true;
      try {
        const url = `${location.origin}/api/v1/dues/generate`;
        const response = await axios.post(url);
        if ([200, 201].includes(response.status)) {
          window.s_alert(response.data?.message ?? "বকেয়া সফলভাবে তৈরি হয়েছে");
          await this.get_all();
        } else {
          window.s_warning(response.data?.message ?? "কিছু সমস্যা হয়েছে");
        }
      } catch (error) {
        const msg = error.response?.data?.message ?? "বকেয়া তৈরি করতে ব্যর্থ হয়েছে";
